...
讲视频编码，实际上还是得从 I, P, B 帧说起，这三种帧是如今普及率最高的 H264 所使用的帧类型。

视频编码一切的一切，基础的基础，所有东西都引用它，因此我们从 I 帧讲起。

I 帧就是存储的一张完整的图片。由于在视频编码过程中需要对一张完整的图片进行编码，于是乎也出现了 HEIF、AVIF 等规范，他们就是利用了 HEVC、AV1 所规定的*帧内编码*技术带来的提升，从而做到极高的压缩率。

这里我们追根溯源，从单张图片的老祖宗——JPEG 说起。JPEG 干的事情，现在的编码器对画面处理的时候全都会做。

首先，它将 RGB 转换成 YUV，然后通过***色度半采样***技术，直接去掉了大量的信息。这里的信息，是指最基础概念的，0 和 1。

然后，JPEG 将图片分为了无数个 8x8 的小块，每一块都是 64 个像素，对这些小块分别进行编码。

这里 JPEG 使用了名为 ***DCT***：***离散余弦变换*** 的技术。DCT的具体数学原理和计算方式，这里我们不做详细介绍，感兴趣的话可以通过[这个教程](https://de.mathworks.com/help/images/discrete-cosine-transform.html)进行了解。

DCT 做的事情，可以简单理解为空间换时间的傅里叶变换，将离散的输入变到频域空间。

下面这张表是 DCT 认为的 8x8 矩阵的基函数，表中每个格子是一个最基础的 8x8 小块。也就是说任何实际中的 8x8 小块，都可以使用这些基础块组合得到。

<img src="./media/DCT_00.png" />

通过记录组合时每个格子用了多少次，我们获得了一个 8x8 的矩阵，这个矩阵就是 DCT 的结果。  
接下来就不必再存储原始的 8x8 小块，只需存储这个 8x8 的 DCT 参数矩阵。在解码时，做一个 DCT 的逆变换，用对应参数乘以表中的基础块，再将它们全部累加，就可以重建出原始的 8x8 小块。

<img src="./media/DCT_01.png" />

JPEG 在计算出 DCT 参数矩阵后，还会将结果除以另一个表格，这个表格叫做*量化表*。将两个矩阵中对应位置相除，就获得了一个 ***量化*** 后的矩阵。

<img src="./media/Quantization-Table.png" />

为什么 JPEG 不直接存储原始的 8x8 小块，而是要转换为 DCT 系数，再进行量化后储存呢？

观察 DCT 系数矩阵可以发现，左上角表示低频部分有较大的数字，而右下角表示高频部分的数字都接近于零。在经过量化以后这部分表示高频的数字则直接被抹为零了，整个矩阵变成了一个稀疏矩阵，这样压缩起来就会高效很多。

之后编码器再使用*霍夫曼编码*、*熵编码*等无损压缩方式，将 8x8 的系数矩阵压缩成为简短的很小的一段数据。  

以上就是 *色度半采样* - *DCT 变换* - *量化* 的编码过程，这一过程中去除了高频数据，只留下低频数据。如果*量化表*非常激进，去除了过多的高频系数，那么在解码做 DCT 逆变换后，重构的图像就会与原始图像有较大的差异。特别是原始图像中，相邻的 8x8 小块之间的差异会被拉大，它们之间的过渡变得不再自然，于是我们就会看到低码率视频最常见的一种瑕疵 **Blocking**。  

总结一下，编码器在帧内编码做的事情，就是把对于人眼不是很敏感的高频数据抹掉。运用这项技术，再加上后续帧间编码所需要的*运动侦测*，就有了最基础的视频编码器。

### 帧间编码 (Inter-frame coding)

作为视频编码器，需要在时域上寻找相似的地方，并且把这些相似的地方标记出来，去找别的帧引用。

怎么分割动与静，怎么将画面细节拿出来，方案一直在进化，标准一直在提升。

当初的 MPEG2，分割是最无脑的分割，也就是纯分块。
...
而再次进化到 H265，块的分割变得更加自由，最小单位也变得灵活。  

<img src="./media/H265.png" />

除了常规的正方形，还出现了 `Rect`、`Amp` 这样非对齐的分割。 
